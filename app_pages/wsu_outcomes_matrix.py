...
def load_data():
    ...
    df = pd.read_parquet('data/app/wsu_cip_outcomes.parquet')
    df_soc = pd.read_parquet('data/app/statewide_soc_benchmarks.parquet')
    return df, df_soc
...
selected_college_code = st.sidebar.selectbox(
    ...
    format_func=lambda x: COLLEGE_NAMES.get(x, x) if x != 'All Colleges' else 'All Colleges'
)
...
# Histogram of cohort sizes (placed after controls)
if not filtered_df.empty:
    ...
    st.sidebar.plotly_chart(fig_hist, width="stretch", config={'displayModeBar': False})

# Sidebar Definitions & Sources at bottom
st.sidebar.markdown("""
---
**Definitions & Sources:**
- **Data Sources:** 
...
if filtered_df.empty:
    ...
    fig = px.scatter(
        ...
        size=size_col,
        color=quad_col,
        symbol=quad_col, # Redundant shape encoding in Plotly scatter
    ...
    with st.expander("♿ Accessible Data Table - WSU CIP Performance Matrix"):
        st.dataframe(
            chart_df[['CIP Code', 'CIPTitle', 'Award', size_col, emp_col, y_col, quad_col]],
            column_config={
                size_col: "Graduates Count",
                emp_col: "Employed %",
                y_col: y_label,
                quad_col: "Quadrant"
            },
